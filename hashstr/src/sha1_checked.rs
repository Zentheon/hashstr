...
use fstr::FStr;
use hashstr_derive::{HashStrWrapper, impl_hashstr_tests};
use sha1_checked::{
    Sha1,
    ...
    ($ident:ident)  => {
        impl From<sha1_checked::CollisionResult> for CollisionResult<$ident> {
            fn from(res: sha1_checked::CollisionResult) -> Self {
                let hashstr = res.hash().into();
                match res {
                    sha1_checked::CollisionResult::Ok(_) => CollisionResult::Ok(hashstr),
                    sha1_checked::CollisionResult::Mitigated(_) => {
                        CollisionResult::Mitigated(hashstr)
                    }
                    sha1_checked::CollisionResult::Collision(_) => {
                        CollisionResult::Collision(hashstr)
                    }
                }
            }
        }
