...
fn main() -> std::io::Result<()> {
    ...
    let hash2 = Sha256Str::digest_reader(&mut reader)?;
    println!("Hash of '42': {hash2}");
    
    #[cfg(no_run)] // This one is omitted from doctests 
    let hash3 = Sha256Str::digest_file("./example.txt")?;
...
let hash_lower = Sha224Str::digest(data);
let hash_upper = Sha224StrUpper::digest(data);
...
let hash: Sha512Str = hasher.finalize().into();
```
